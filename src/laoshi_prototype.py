...
class AudioStatusIndicator(Static):
    ...
    def render(self) -> str:
        status = (
            "🔴 Microphone on (Press Q to quit)" if self.is_recording else "⚪ Press the spacebar to activate microphone (Q to quit)"
        )
        return status
...
class RealtimeApp(App[None]):
    ...
    async def handle_realtime_connection(self) -> None:
        async with self.client.realtime.connect(model=LARGE_LANGUAGE_MODEL) as conn:
            ...
            await conn.session.update(
                session={
                    ...
                    "type": "realtime",
                    "instructions": "You are an interactive Chinese language tutor. When the session starts, \
        ...
        otherwise). Don't ask the student to say the same phrase more than 4 times in a row, unless the \
        student's pronounciation is unintelligible or the student requests to continue practicing that \
        phrase. If the student's pronounciation is bad or unintelligible or the student requests to continue \
        practicing that phrase, continue with that phrase. \
        Here is the list of HSK-3 words in Traditional Chinese: \
    ...
    async def on_key(self, event: events.Key) -> None:
        ...
        if event.key == "space" and time.time() > self.record_button_timestamp + 0.3:
            ...
            if status_indicator.is_recording:
                await asyncio.sleep(0.39) # Give the LLM time to send a response
